    ...
    async connect(decryptPermission: DecryptPermission, network: WalletAdapterNetwork, programs?: string[]): Promise<void> {
        try {
            ...
            try {
                ...
                // Puzzle Wallet may return the public key from connect
                let result;
                ...
                if (!publicKey) {
                    throw new WalletConnectionError('Failed to get public key from Puzzle Wallet. Please make sure you approved the connection in the wallet extension.');
                }
                ...
                throw new WalletConnectionError(error?.message || 'Failed to connect to Puzzle Wallet', error);
            }
